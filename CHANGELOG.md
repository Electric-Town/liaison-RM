...
- Parallel P04 typed React foundation with an exact-version toolchain, versioned application-response contract, injected Tauri transport seam, stable B0 routes and Events stages, explicit P03 operation phases, cross-platform tests, and no browser-storage or network authority.
- Canonical P03D Editorial Ledger experience contract with versioned semantic tokens, typed operation-state presentation, five B0 routes, five-stage Events flow, accessibility and localization evidence requirements, and the reviewed six-slice P04 migration plan.
