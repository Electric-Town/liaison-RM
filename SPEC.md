...
The exact working-state boundary and accepted delivery order are maintained in [`docs/product/working-state-delivery.md`](docs/product/working-state-delivery.md). The current P03 source state contains a narrow Workspace/People/Markdown slice behind one `liaison-application` composition root, write-authoritative `WorkspaceSession` values with composite path-local and per-user `WorkspaceId` operating-system exclusion, retained capability roots, lock-free read-only Health, structured CLI/Tauri results and errors, profile and reason-only Review domain foundations, and provider contracts with a limited local-folder reference adapter. Cooperating Liaison processes on one user account and machine reject an independently copied workspace with the same identity. `T-B0-P02` is complete with accepted exact-head Linux, macOS, and Windows evidence. Baseline `3499a6e9278fc72d2498a9978df59f30d03722e6` contains the P03 recoverable-operation candidate, including multi-target commits and final mutation preconditions; all seven ordinary-push workflows succeeded, but P03 remains technically unaccepted until its hardening, qualification, pairwise-distinct qualified-code/merge-result/attestation identities, qualification receipt, and exact executable artifact receipt complete. Separately dispatched notarized-bundle run `29899498005` failed for missing Apple credentials and is not release evidence. The reviewed pre-reconciliation main at 2026-07-22, `49ee419e30f2d71524dd6fa15badf1ec4b8d0e27`, adds unaccepted out-of-order source and has red exact-head formatting checks; neither that history nor its unsupported `vB0` tag advances P03, OBS, P03D, P04, P05-P11, or B0. The C3 commit and any merge result are distinct identities requiring their own exact-head receipt; absent that receipt they do not advance acceptance. The source state still does **not** contain an accepted event workflow, sealed dietary persistence, local grant enforcement, an encrypted recovery package, a proven Airgap artifact, or a supported public release.
...
The current pre-alpha shell is directly committed HTML/CSS/JavaScript and is disposable. Its restored `49ee419` pages, themes, and fixtures are candidate source only and do not implement or accept P04; the React/TypeScript/Vite target remains blocked until P03 technical acceptance, D1-B Continue, and P03D. P04 migrates the shell once to that typed inbound adapter and proves Workspace, People, and Health parity plus the semantic token/component and current-surface built-in-theme matrix through the same Rust application commands. P04 does not own Events or the full B0 theme journey. P11 applies that foundation to Overview, Directory, Events, Health, Settings, and the installed Details-to-Brief workflow in every built-in. JavaScript does not own canonical persistence, authorization, readiness, recovery, or a second domain model.
...
- General and third-party post-A0 migrations use deterministic dry-run, pre-migration backup, validation, and rollback or explicit irreversibility under `LRM-WS-007`; the narrow B0 OKF People normalization is separately and exclusively governed by `LRM-WS-017` and `UAT-066`.
...
5. **P03 observation checkpoint (D1-B/D9):** after technical P03 qualification is accepted with qualified-code, merge-result, attestation, and exact-executable identities, observe that same artifact with synthetic or redacted workplace scenarios. Record Continue, Change, or Stop; P03 technical acceptance does not itself authorize design.
6. **P03 design gate:** only Continue makes P03D eligible. Design consultation creates canonical `DESIGN.md`; plan design review checks it against the complete B0 journey and amends P04 before desktop implementation. Change, Stop, or pending observation keeps P03D and P04 blocked. G0 records but does not execute this gate.
7. **P04 — desktop adapter migration:** React/TypeScript/Vite over typed Rust commands with Workspace/People/Health parity, the semantic token/component foundation, and installed current-surface theme/recovery evidence. It does not own Events or the full B0 journey.
8. **P05 — G1 domain contracts plus atomic OKF seam:** after completed P04, the G1 P05 task defines revisioned People, Organisations, Groups, Locations, Memberships, Events, provenance, field state, and event-local resolution types before P06/P07; separate `T-B0-P05-OKF` owns the pinned OKF v0.1 Draft schema and strict People authoring port under `FG-B0-001`. Sensitive contracts are not owned by either P05 task and remain exclusive to P07/P08 under `FG-B0-002`; G3 consumes the accepted P05 contracts.
9. **P06 — scalable tolerant Directory reads:** the single Directory projection owns OKF-tolerant reading, Liaison domain-validity quarantine, SQLite/FTS filters, pagination, canonical revalidation, Health findings, and 10,000/50,000 evidence.
10. **P06-REPAIR — guided canonical repair:** `T-B0-P06-REPAIR` combines P03 recoverable operations with P06 tolerant findings to preview duplicate-ID and invalid-record repair, create an exact pre-repair backup, apply failure-atomically, record an exact receipt, and support exact rollback before P09-OKF.
11. **P07 — Workspace Security and local policy:** key lifecycle, recovery envelope, optional Keychain cache, trusted local owner/device, purpose grants, role presets, and payload-minimal activity evidence.
12. **P08 — recovery before real sensitive data:** quiescent local checkpoint plus encrypted clean-install recovery package.
13. **P09 — required OKF normalization then Directory onboarding:** `T-B0-P09-OKF` owns previewable, exact-backup-first, journaled, failure-atomic, idempotent and exactly reversible normalization after P03/P05-OKF/P06/P06-REPAIR and closes `FG-B0-001`. The separate G3 P09 task then owns People maintenance and streaming CSV preview/reconciliation for People, Organisations, Locations, Groups, and Memberships.
14. **P10 — Events core:** immutable cohort, exact readiness, structurally limited `DietaryOperationalView`, sealed internal brief, verified delivery, and staleness.
15. **P11 — B interface:** Overview, Directory, Events, Health, and Settings with built-in theme choice and persistence only, plus the complete cohort-to-brief state machine and installed every-built-in theme matrix; settings bundle transfer begins in A0.
16. **B0 — Workplace Review Alpha:** for one trusted local workspace owner, the installed universal Mac review artifact passes scale, crash, repair, key, grant, leak, encrypted-restore, accessibility, offline, readable-file, and contributor-journey evidence. Workplace surfaces structurally omit relationship allocation/ranking/scoring. It remains an internal review alpha unless public signing/notarisation gates pass.
17. **A0 — Personal Memory Alpha:** only after B0 acceptance, add quick/full capture, a source-complete purpose-scoped profile, stable user-organised tabs/layouts with lossless settings round trips, explicit fact states, reversible identity review, a source- and range-labelled unified timeline, meaningful interactions, bounded commitments, distinct last-note/last-interaction values, open-loop views, and reason-only Review over the same foundations. A0 does not add a generic task engine, global person score, or automatic exact/fuzzy merge.
18. **Post-A0:** sharing, provider transports, contacts/calendars/email, facilities, mobile products, Meitheal integration, OpenAPI/MCP/AI/plugins, Linux/Windows support, and public notarized distribution advance under their own gates.
...
This normative specification is the identifier-exact authority for:

- `cross-release::LRM-PK-008`
