...
Status: candidate evidence for `T-B0-P03D` and the P04 token work; no production or conformance claim.

This document and its companion files exist to discharge one accepted-review obligation ahead of P04: the review rejected the dark highlight pairing at about 1.77:1 and required theme-specific `content-on-highlight` and `current-step-content` tokens. The correction is recorded here as measured arithmetic, not as a screenshot judgement.

What this is not:

- It is not the P04 semantic-token registry; P04 creates that under the canonical `DESIGN.md` the post-P03 design consultation produces. This is one prepared input.
- Token **names** follow the review's normative roles; every **value** is provisional — taken verbatim from the approved Editorial Ledger candidate palette of 2026-07-19 except the two correction tokens — and may change when measured on the exact P04 build without reopening the visual direction.
...
- `scripts/check_design_tokens.py` — the validator: WCAG 2.x relative-luminance contrast for every declared pair in every theme; fails on any required pair below its minimum. Its self-test first reproduces the review's rejected pair so the arithmetic is anchored to the measurement that caused the rejection.

## The rejected pair and its correction

| Pairing | Measured | Verdict |
|---|---:|---|
| `#17231F` content on `#514819` highlight (the reviewed dark preview) | 1.77:1 | Rejected by the review; reproduced by the validator's self-test |
...
The correction keeps the approved olive highlight in dark and fixes only the content placed on it: light ink, not near-black. `current-step-content` mirrors the same values so the Event stepper's current stage never inherits an unreadable pairing.
...
Informational pairs the validator flags rather than fails — recorded as findings, not silently corrected, because the values are the approved palette's:
...
4. **High-contrast values are candidate placeholders more than the others**: the approved candidate specifies high-contrast sparsely, so several surface tokens reuse `#101010`. The consultation should confirm or replace them, and forced-colours behaviour is untested arithmetic-free territory until the exact build.

## Typography roles and provenance obligations

Roles are normative per the accepted review; nothing here vendors a font:
