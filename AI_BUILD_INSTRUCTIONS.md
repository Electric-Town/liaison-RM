...
Execution position: G0/P00 and P01 are complete on the default branch; P02 is also complete, G1 is current, and P03 is the active package. PR #65 established baseline `3499a6e9278fc72d2498a9978df59f30d03722e6`; all seven ordinary-push workflows succeeded in runs `29899084738`, `29899084740`, `29899084741`, `29899084751`, `29899084753`, `29899084769`, and `29899084789`, including Windows. Separately dispatched notarized run `29899498005` failed for missing Apple credentials and is not release evidence. The baseline contains implemented P03 candidate source, but P03 technical acceptance remains pending. D1-B is selected: accepted P03 qualification makes OBS current while P03D remains blocked, and only a separate Continue decision makes P03D eligible.

The reviewed pre-reconciliation main at 2026-07-22, `49ee419e30f2d71524dd6fa15badf1ec4b8d0e27`, contains later candidate source but has red exact-head Rust/Windows formatting checks and no accepted P03 identity tuple or OBS receipt. Preserve that lineage and the unsupported historical `vB0` tag, but do not use their code, `DESIGN.md`, `.planning` archive, static UI, P04 reviews, installed app, or tag as task/gate evidence. No P03 source fix, D1-B session, P03D/P04 work, or runnable-UI correction may start until the authority reconciliation is independently accepted. The C3 commit and any merge result are distinct identities requiring their own exact-head receipt; absent that receipt they do not advance acceptance.
...
4. **P03 — recoverable operations:** harden and qualify the implemented candidate staged multi-target operations with a durable commit decision and roll-forward recovery. Technical acceptance must output the qualified-code SHA, merge-result SHA, attestation SHA, and exact executable artifact receipt; it does not close broader `FG-B0-001`.
5. **P03 observation checkpoint (D1-B/D9):** only after technical P03 acceptance, run `T-B0-P03-OBS` against that same exact executable with synthetic or redacted workplace scenarios. Record a distinct Continue, Change, or Stop decision; real workplace personal data is denied. Technical acceptance is not the observation decision, and only Continue advances.
6. **P03 design gate:** after Continue, run design consultation to create canonical `DESIGN.md`, then run plan design review against the complete B0 journey and amend P04 before implementation. G0 records this gate but does not create the file or preselect the direction.
7. **P04 — desktop inbound adapter:** migrate to React/TypeScript/Vite over typed Rust commands and prove Workspace, People, and Health parity plus the installed current-surface system/light/dark/high_contrast, OS-following, interruption, recovery, en-XA, VoiceOver, 400% reflow, and zero-egress matrix. P04 does not own Events, UAT-062, or A0 relationship reminders.
8. **P05/P05-OKF/P06 — portable domain and People foundation:** after completed P04, establish P05's Directory/Event/dietary contracts in G1 and P05-OKF's pinned OKF v0.1 Draft strict writer/schema port as an `FG-B0-001` slice. P05 unlocks P07; P05 together with P05-OKF unlocks P06's tolerant Directory reader and domain-validity quarantine, which is accepted by `FG-R1-003`. P09-OKF is the later acceptance task that closes `FG-B0-001`. Do not expand P01/P02.
9. **P06-REPAIR — guided canonical repair:** `T-B0-P06-REPAIR` runs after P03 and P06, previews invalid-record repair, creates an exact backup, applies through failure-atomic recoverable operations, records an exact receipt, supports exact rollback, and closes `UAT-040` under `FG-R1-002` before P09-OKF.
10. **P07/P08 — separate security foundations:** keep sensitive types/policy exclusively in P07 under `FG-B0-002`, then prove checkpoint and encrypted clean-install recovery in P08; G3 consumes the earlier P05 contracts rather than preceding these G1 tasks.
11. **P09-OKF/P09–P11 — B product:** after P06-REPAIR, add required previewable, exact-backup-first, journaled, failure-atomic, idempotent and exactly reversible People normalization and close `FG-B0-001`, then complete Directory onboarding import, Events/cohort/readiness/brief workflows, and the installed desktop every-built-in Details-to-Brief experience under UAT-062.
12. **B0 qualification:** with one trusted local workspace owner, run scale, crash, repair, key, grant, leak, OKF, normalization/rollback, accessibility, offline, installed-app, and developer-journey evidence on the exact review artifact. Workplace domain types and outputs must structurally omit relationship allocation, ranking, and scoring.
13. **A0:** only after B0 acceptance, implement quick/full capture, the source-complete purpose-scoped profile, explicit fact states, reversible identity review, source/range timeline, stable custom-field layouts and user-organised profile tabs, interactions, bounded commitments, reason-only Review, and the personal-memory journey. Do not introduce a generic task engine, global person score, or automatic merge.

General and third-party migrations are excluded from B0 except for the required OKF People normalization. Generic migration safety `LRM-WS-007` belongs to R5 `T-R5-005`/`FG-R5-005`, not P03 or P09-OKF. Mobile products, provider transports, multi-writer sharing, AI/MCP, Meitheal integration, broad platform support, and public notarized distribution remain later gates unless a prerequisite contract explicitly requires a narrow seam. Later providers have no hidden sync, refresh, or egress; later AI, MCP, plugins, providers, and imports can stage source-backed proposals but cannot write confirmed facts directly.
